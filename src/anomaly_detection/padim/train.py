...
from src.anomaly_detection.padim.config import DEFAULT_CONFIG_PATH, load_experiment_config
from src.anomaly_detection.padim.model import build_padim_model


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train PaDiM on VisA.")
...
def build_datamodule(config, *, category: str, image_size: int, backbone: str, val_split_ratio: float):
    del backbone
...
def run_single(config, *, output_dir: Path, backbone: str, image_size: int, n_features: int | None) -> dict:
    ...
    for category in get_categories(config):
        datamodule = build_datamodule(config, category=category, image_size=image_size, backbone=backbone, val_split_ratio=config.train.val_split_ratio)
...
def run_test(config, best_artifact_path: Path, *, model_params: dict | None = None) -> dict:
    ...
    n_features = params.get("n_features", config.model.n_features)
...
def run_grid_tuning(config) -> pd.DataFrame:
    ...
    for backbone in config.grid.backbones:
        for image_size in config.grid.image_sizes:
            for n_features in config.grid.n_features_list:
                trial_name = f"{backbone}_sz{image_size}_nf{n_features}"
                result = run_single(config, output_dir=tuning_root / trial_name, backbone=backbone, image_size=image_size, n_features=n_features)
...
def main() -> None:
    ...
    setup_dtd_symlink(
        dtd_data_root=Path(config.dataset_root).parent,
    ...
    if config.tuning_mode == "grid":
        ...
        summary = {
            ...
            "best_model_config": {
                ...
                "n_features": None if pd.isna(best.get("n_features")) else int(best["n_features"]),
            },
        ...
        result = run_single(
            config,
            output_dir=config.output_path / "train",
...
def _save_run_results(output_dir: Path, category_frames: list, pixel_rows: list, train_split_rows: list[dict] | None = None, **extra) -> dict:
    ...
    return {
        ...
        "image_auroc": float(image_metrics["image_auroc"]),
