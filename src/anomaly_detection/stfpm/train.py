...
from src.anomaly_detection.stfpm.config import DEFAULT_CONFIG_PATH, load_experiment_config
from src.anomaly_detection.stfpm.model import build_stfpm_model


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train STFPM on VisA.")
...
def run_single(
    ...
    *,
    ...
    for category in get_categories(config):
        ...
        model = build_stfpm_model(backbone=backbone, layers=layers, pre_trained=config.model.pre_trained, image_size=image_size)
...
def run_test(config, best_artifact_path: Path, *, model_params: dict | None = None) -> dict:
    ...
    layers = params.get("layers", config.model.layers)
    image_size = int(params.get("image_size", config.train.image_size))

    for category in get_categories(config):
        datamodule = build_datamodule(config, category=category, image_size=image_size)
        model = build_stfpm_model(
            backbone=backbone,
            layers=layers,
...
def run_grid_tuning(config) -> pd.DataFrame:
    ...
    for backbone in config.grid.backbones:
        for image_size in config.grid.image_sizes:
            for layers in config.grid.layers_list:
                layers_str = "_".join(layers)
                trial_name = f"{backbone}_sz{image_size}_{layers_str}"
                result = run_single(config, output_dir=tuning_root / trial_name, backbone=backbone, layers=layers, image_size=image_size)
...
def run_optuna_tuning(config) -> pd.DataFrame:
    ...
    layers_candidates = config.optuna.layers_candidates or config.grid.layers_list
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        layers_idx = trial.suggest_categorical("layers_idx", list(range(len(layers_candidates))))
        layers = layers_candidates[layers_idx]
        trial_name = f"trial_{trial.number:03d}"
        try:
            result = run_single(config, output_dir=output_dir / trial_name, backbone=backbone, layers=layers, image_size=image_size, optuna_trial=trial)
...
def main() -> None:
    ...
    if config.tuning_mode == "full":
        ...
        result = run_single(config, output_dir=config.output_path / "train", backbone=config.model.backbone, layers=config.model.layers, image_size=config.train.image_size)
        summary = {
            "experiment_name": config.experiment_name,
            "mode": "none",
            "best_source": "train",
