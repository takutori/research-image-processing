...
from src.classification.eva02.config import (
    DEFAULT_CONFIG_PATH,
    EVA02ExperimentConfig,
    load_experiment_config,
)
from src.classification.eva02.model import build_eva02_model


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train EVA-02 with config-driven pipeline.")
...
def infer_fixed_image_size(model_name: str) -> int | None:
    match = re.search(r"_(\d{3,4})$", model_name)
    if match is None:
        return None
...
def validate_image_size_for_model(model_name: str, image_size: int) -> None:
    expected_image_size = infer_fixed_image_size(model_name)
    if expected_image_size is None:
        return
    if image_size != expected_image_size:
        raise ValueError(
            f"{model_name} expects image_size={expected_image_size}, "
            f"but got image_size={image_size}. "
            "Update the config or choose a model variant whose suffix matches the input size."
        )


def train_single_run(
    config: EVA02ExperimentConfig,
    output_dir: Path,
    *,
    image_size: int,
...
def run_plain_training(config: EVA02ExperimentConfig, device: str) -> dict[str, object]:
...
def run_grid_tuning(config: EVA02ExperimentConfig, device: str) -> pd.DataFrame:
...
def run_optuna_tuning(config: EVA02ExperimentConfig, device: str) -> pd.DataFrame:
    ...
    fixed_image_size = infer_fixed_image_size(config.model_name)
    if config.optuna.image_size_candidates is not None:
        image_size_candidates = config.optuna.image_size_candidates
    elif fixed_image_size is not None:
        image_size_candidates = [fixed_image_size]
    else:
        image_size_candidates = [best_image_size]
...
def run_full_tuning(config: EVA02ExperimentConfig, device: str) -> dict[str, object]:
