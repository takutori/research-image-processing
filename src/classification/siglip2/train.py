...
from src.classification.siglip2.config import DEFAULT_CONFIG_PATH, SigLIP2ExperimentConfig, load_experiment_config
from src.classification.siglip2.model import build_siglip2_model, build_siglip2_processor


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train SigLIP 2 with config-driven pipeline.")
...
def infer_fixed_image_size(model_name: str) -> int | None:
    match = re.search(r"-(\d{3,4})$", model_name)
    return int(match.group(1)) if match else None


def validate_image_size_for_model(model_name: str, image_size: int) -> None:
    expected = infer_fixed_image_size(model_name)
    if expected is not None and expected != image_size:
        raise ValueError(
            f"{model_name} expects image_size={expected}, but got image_size={image_size}. "
            "Update the config or choose a compatible checkpoint."
        )


def train_single_run(
    config: SigLIP2ExperimentConfig,
    output_dir: Path,
    *,
    image_size: int,
    ...
    validate_image_size_for_model(config.model_name, image_size)
    ...
    processor = build_siglip2_processor(config.model_name)
    ...
    model = build_siglip2_model(num_classes=len(label_mapping), model_name=config.model_name).to(device)
...
def evaluate_checkpoint(
    ...
    *,
    ...
    processor = build_siglip2_processor(str(config["model_name"]))
    collate_fn = build_hf_processor_collate_fn(processor)
    ...
    model = build_siglip2_model(num_classes=len(label_mapping), model_name=str(config["model_name"]))
...
def run_grid_tuning(config: SigLIP2ExperimentConfig, device: str) -> pd.DataFrame:
    ...
    for idx, params in enumerate(trials, start=1):
        ...
        result = train_single_run(
            ...
            eval_resize_size=int(params["image_size"]),
...
def run_optuna_tuning(config: SigLIP2ExperimentConfig, device: str) -> pd.DataFrame:
    ...
    batch_size_candidates = config.optuna.batch_size_candidates or sorted({max(8, best_batch_size // 2), best_batch_size, best_batch_size * 2})
...
def run_full_tuning(config: SigLIP2ExperimentConfig, device: str) -> dict[str, object]:
...
def run_plain_training(config: SigLIP2ExperimentConfig, device: str) -> dict[str, object]:
