...
from src.classification.florence2.config import DEFAULT_CONFIG_PATH, load_experiment_config
from src.classification.florence2.train import evaluate_checkpoint


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate best Florence-2 checkpoint from config.")
...
def main() -> None:
    ...
    if not summary_path.exists():
        raise FileNotFoundError(f"experiment summary not found: {summary_path}. Run train.py with this config first.")
