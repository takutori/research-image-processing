...
DEFAULT_CONFIG_PATH = Path("/workspace/config/classification/florence2_oxford_pet_fewshot.yaml")
DEFAULT_PROMPT = "<CAPTION>"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train few-shot Florence-2 classifier.")
...
def evaluate_checkpoint(
    ...
    *,
    ...
    device: str,
    model_name: str,
    subset: dict[str, int | None] | None = None,
) -> dict[str, object]:
    checkpoint = torch.load(checkpoint_path, map_location="cpu")
    label_mapping = checkpoint["label_mapping"]
    processor = build_florence2_processor(model_name)
    ...
    if subset is not None:
        test_df = limit_dataframe_examples(test_df, subset.get("test_examples"), random_state=42)
    ...
    model = build_florence2_model(num_classes=len(label_mapping), model_name=model_name)
...
def main() -> None:
    ...
    train_df = limit_dataframe_examples(train_df, subset.get("train_examples"), random_state=raw["train"]["random_state"])
    valid_df = limit_dataframe_examples(valid_df, subset.get("valid_examples"), random_state=raw["train"]["random_state"])
    ...
    processor = build_florence2_processor(raw["model_name"])
    collate_fn = build_hf_processor_collate_fn(processor, prompt_text=DEFAULT_PROMPT)
    ...
    model = build_florence2_model(num_classes=len(label_mapping), model_name=raw["model_name"]).to(args.device)
